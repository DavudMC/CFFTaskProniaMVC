...
using WebApplicationPronia.ViewModels.CloudinaryViewModels;
...
namespace WebApplicationPronia.Services
{
    public class CloudinaryService : ICloudinaryService
    {
        ...
        public CloudinaryService(IConfiguration configuration)
        {
            ...
            _options = configuration.GetSection("CloudinarySettings").Get<CloudinaryOptionsVM>() ?? new();
        ...
        public async Task<bool> FileDeleteAsync(string filePath)
        {
            try
            {
                string publicIdWithExtension = filePath.Substring(filePath.LastIndexOf("MPA101Pronia"));
